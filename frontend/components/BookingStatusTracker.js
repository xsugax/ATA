    ...
    <div className="glass rounded-2xl p-4 xl:sticky xl:top-6">
      ...
      <ol className="space-y-2">
        {stages.map((stage) => {
          ...
          return (
            <li key={stage} className={`rounded-lg border px-3 py-2 text-sm leading-snug ${active ? "border-sovereign bg-sovereign/20 text-sovereign" : "border-white/10 bg-white/5 text-platinum/70"}`}>
              {stage}
            </li>
          );
        })}
